...
const Form = ({ getTasks, onEdit, setOnEdit }) => {
  ...
  useEffect(() => {
    if (onEdit) {
      ...
      task.deadline.value = new Date(onEdit.deadline)
        .toISOString()
        .slice(0, 10);
  ...
  const handleSubmit = async (e) => {
    ...
    if (task.price.value > maxCost) {
      return toast.error(
        `O custo não pode ser maior que R$${maxCost.toLocaleString()}.`
      );
    }
    ...
    if (onEdit) {
      await axios
        .put("https://api-taskly-self.vercel.app/" + onEdit.id, {
      ...
      await axios
        .post("https://api-taskly-self.vercel.app/", {
          name: task.name.value,
          price: task.price.value,
          deadline: task.deadline.value,
          order: task.order.value,
  ...
  const checkTaskName = async (name, id) => {
    try {
      const response = await axios.get("https://api-taskly-self.vercel.app/");
      const existingTasks = response.data;

      return existingTasks.some((task) => task.name === name && task.id !== id);
    } catch (error) {
    ...
    <S.Container>
      <div>
        ...
        <S.FormContainer ref={ref} onSubmit={handleSubmit}>
          ...
          <S.Buttons
            style={{
              alignItems: nameLength >= maxLength ? "center" : "end",
              marginBottom: nameLength >= maxLength ? "0" : "3px",
            }}
          >
            <S.Button title="Salvar tarefa" type="submit">
              Salvar
            </S.Button>
            {onEdit && (
